...
contract Works {
    ...
    uint256 public _nowTime; //TODO:delete code
    function updateNowTime(uint256 _time) external {
        _nowTime = _time;
    }
    ...
    function getDebrisPrice(bytes32 _worksID, uint8 _debrisID) external view returns(uint256) {        
        ...
        if(debris[_worksID][_debrisID].buyNum > 0 && debris[_worksID][_debrisID].lastTime.add(discountGap) < _nowTime) { 

            uint256 n = (_nowTime.sub(debris[_worksID][_debrisID].lastTime.add(discountGap))) / discountGap; 
            if((_nowTime.sub(debris[_worksID][_debrisID].lastTime.add(discountGap))) % discountGap > 0) { 
                n = n.add(1);
            }
